...
p(x) = m1 * x/1e8 +b1
fit p(x) '-' using 1:3 via m1,b1
h(x) = m2 * x/1e8 +b2
fit h(x) '-' using 1:2 via m2,b2

set terminal png enhanced size 1024,768
plot '-' using 1:2 notitle with boxes lc rgb "blue", \
p(x) title 'Promise count trend' with lines linestyle 1, \
h(x) title 'Host count trend' with lines linestyle 2
END
...
END
   ...
   {
      say DATA_FILE $k." ". $param{'data'}->{$k}->{hosts}.
         $param{'data'}->{$k}->{$y_axis};
   }
   close DATA_FILE;

   open (GNUPLOT, "| $gnuplot") or die "Cannot pipe to $gnuplot";
   say GNUPLOT $plot_cmd;
   close GNUPLOT;
